...
void CCommon::Save_MotionPosIO()
{
	...
	gAlm.bStage[12] = pDX03->iTransferZUp;
	gAlm.bStage[13] = pDX03->iBTMMirrorIn;
	gAlm.bStage[14] = pDX03->iAlignMasterIn;
	gAlm.bStage[15] = pDX03->iAlignSlave1In;
	gAlm.bStage[16] = pDX03->iAlignSlave2In;
}
...
BOOL CCommon::Check_StageUpDown(CString &sMsg)
{
	...
	CString sStage[17] = {"Load Stage1","Load Stage2","Inspection Stage1","Inspection Stage2","Inspection Stage3","Inspection Stage4","Inspection Stage1","Inspection Stage4","NG Stage1","NG Stage2","Good Stage1","Good Stage2","Transfer1 Z","BTM Mirror","Align Master","Align Slave1","Align Slave2"};
	...
	if (gAlm.bStage[12] != pDX03->iTransferZUp)			nErrNo = 13;
	if (gAlm.bStage[13] != pDX03->iBTMMirrorIn)			nErrNo = 14;
	if (gAlm.bStage[14] != pDX03->iAlignMasterIn)		nErrNo = 15;
	if (gAlm.bStage[15] != pDX03->iAlignSlave1In)		nErrNo = 16;
	if (gAlm.bStage[16] != pDX03->iAlignSlave2In)		nErrNo = 17;

	if (nErrNo > 0) {
		if (nErrNo == 7 || nErrNo == 8 || nErrNo == 14 || nErrNo == 15 || nErrNo == 16 || nErrNo == 17)
...
BOOL CCommon::Check_Position(int nAxis, int nMoveIdx, double dRange)
{
	...
	if (g_objAJinAXL.Is_Done(nAxis)) {
		...
		if (nAxis == AX_TRANSFER_X2)		dCheckPos = pMoveData->dTransferX2[nMoveIdx];		// AXIS 08
		if (nAxis == AX_TRANSFER_Z2)		dCheckPos = pMoveData->dTransferZ2[nMoveIdx];		// AXIS 09
		if (nAxis == AX_LOAD_STAGE_Y1)		dCheckPos = pMoveData->dLoadStageY1[nMoveIdx];		// AXIS 10
		if (nAxis == AX_LOAD_STAGE_Y2)		dCheckPos = pMoveData->dLoadStageY2[nMoveIdx];		// AXIS 11
		if (nAxis == AX_LOAD_PICKER_X1)		dCheckPos = pMoveData->dLoadPickerX1[nMoveIdx];		// AXIS 12
		if (nAxis == AX_LOAD_PICKER_Y1)		dCheckPos = pMoveData->dLoadPickerY1[nMoveIdx];		// AXIS 13
		if (nAxis == AX_LOAD_PICKER_Z1)		dCheckPos = pMoveData->dLoadPickerZ1[nMoveIdx];		// AXIS 14
		if (nAxis == AX_LOAD_PICKER_P1)		dCheckPos = pMoveData->dLoadPickerP1[nMoveIdx];		// AXIS 15
		if (nAxis == AX_LOAD_PICKER_X2)		dCheckPos = pMoveData->dLoadPickerX2[nMoveIdx];		// AXIS 16
		if (nAxis == AX_LOAD_PICKER_Y2)		dCheckPos = pMoveData->dLoadPickerY2[nMoveIdx];		// AXIS 17
		if (nAxis == AX_LOAD_PICKER_Z2)		dCheckPos = pMoveData->dLoadPickerZ2[nMoveIdx];		// AXIS 18
		if (nAxis == AX_LOAD_PICKER_P2)		dCheckPos = pMoveData->dLoadPickerP2[nMoveIdx];		// AXIS 19
		if (nAxis == AX_BTM_FOCUS_Z)		dCheckPos = pMoveData->dBTMFocusZ[nMoveIdx];		// AXIS 20
		if (nAxis == AX_BTM_LIGHT_Z)		dCheckPos = pMoveData->dBTMLightZ[nMoveIdx];		// AXIS 21
		if (nAxis == AX_TOP1_VISION_Z)		dCheckPos = pMoveData->dTOPVisionZ1[nMoveIdx];		// AXIS 22
		if (nAxis == AX_TOP2_VISION_Z)		dCheckPos = pMoveData->dTOPVisionZ2[nMoveIdx];		// AXIS 23
		if (nAxis == AX_INSPECT_STAGE_X1)	dCheckPos = pMoveData->dVisionStageX1[nMoveIdx];	// AXIS 24
		if (nAxis == AX_INSPECT_STAGE_X2)	dCheckPos = pMoveData->dVisionStageX2[nMoveIdx];	// AXIS 25
		if (nAxis == AX_INSPECT_STAGE_X3)	dCheckPos = pMoveData->dVisionStageX3[nMoveIdx];	// AXIS 26
		if (nAxis == AX_INSPECT_STAGE_X4)	dCheckPos = pMoveData->dVisionStageX4[nMoveIdx];	// AXIS 27
		if (nAxis == AX_UNLOAD_PICKER_X1)	dCheckPos = pMoveData->dUnloadPickerX1[nMoveIdx];	// AXIS 28
		if (nAxis == AX_UNLOAD_PICKER_Y1)	dCheckPos = pMoveData->dUnloadPickerY1[nMoveIdx];	// AXIS 29
		if (nAxis == AX_UNLOAD_PICKER_Z1)	dCheckPos = pMoveData->dUnloadPickerZ1[nMoveIdx];	// AXIS 30
		if (nAxis == AX_UNLOAD_PICKER_P1)	dCheckPos = pMoveData->dUnloadPickerP1[nMoveIdx];	// AXIS 31
		if (nAxis == AX_UNLOAD_PICKER_X2)	dCheckPos = pMoveData->dUnloadPickerX2[nMoveIdx];	// AXIS 32
		if (nAxis == AX_UNLOAD_PICKER_Y2)	dCheckPos = pMoveData->dUnloadPickerY2[nMoveIdx];	// AXIS 33
		if (nAxis == AX_UNLOAD_PICKER_Z2)	dCheckPos = pMoveData->dUnloadPickerZ2[nMoveIdx];	// AXIS 34
		if (nAxis == AX_UNLOAD_PICKER_P2)	dCheckPos = pMoveData->dUnloadPickerP2[nMoveIdx];	// AXIS 35
		if (nAxis == AX_GOOD_STAGE_Y1)		dCheckPos = pMoveData->dGoodStageY1[nMoveIdx];		// AXIS 36
		if (nAxis == AX_GOOD_STAGE_Y2)		dCheckPos = pMoveData->dGoodStageY2[nMoveIdx];		// AXIS 37
		if (nAxis == AX_NG_STAGE_Y1)		dCheckPos = pMoveData->dNGStageY1[nMoveIdx];		// AXIS 38
		if (nAxis == AX_NG_STAGE_Y2)		dCheckPos = pMoveData->dNGStageY2[nMoveIdx];		// AXIS 39
		if (nAxis == AX_SHIP_ALIGN_X)		dCheckPos = pMoveData->dShiipAling[nMoveIdx];		// AXIS 40
...
void CCommon::Move_Position(int nAxis, int nMoveIdx, double dSpeed)
{
	...
	if (nAxis == AX_TRANSFER_X2)		dMovePos = pMoveData->dTransferX2[nMoveIdx];		// AXIS 08
	if (nAxis == AX_TRANSFER_Z2)		dMovePos = pMoveData->dTransferZ2[nMoveIdx];		// AXIS 09
	if (nAxis == AX_LOAD_STAGE_Y1)		dMovePos = pMoveData->dLoadStageY1[nMoveIdx];		// AXIS 10
	if (nAxis == AX_LOAD_STAGE_Y2)		dMovePos = pMoveData->dLoadStageY2[nMoveIdx];		// AXIS 11
	if (nAxis == AX_LOAD_PICKER_X1)		dMovePos = pMoveData->dLoadPickerX1[nMoveIdx];		// AXIS 12
	if (nAxis == AX_LOAD_PICKER_Y1)		dMovePos = pMoveData->dLoadPickerY1[nMoveIdx];		// AXIS 13
	if (nAxis == AX_LOAD_PICKER_Z1)		dMovePos = pMoveData->dLoadPickerZ1[nMoveIdx];		// AXIS 14
	if (nAxis == AX_LOAD_PICKER_P1)		dMovePos = pMoveData->dLoadPickerP1[nMoveIdx];		// AXIS 15
	if (nAxis == AX_LOAD_PICKER_X2)		dMovePos = pMoveData->dLoadPickerX2[nMoveIdx];		// AXIS 16
	if (nAxis == AX_LOAD_PICKER_Y2)		dMovePos = pMoveData->dLoadPickerY2[nMoveIdx];		// AXIS 17
	if (nAxis == AX_LOAD_PICKER_Z2)		dMovePos = pMoveData->dLoadPickerZ2[nMoveIdx];		// AXIS 18
	if (nAxis == AX_LOAD_PICKER_P2)		dMovePos = pMoveData->dLoadPickerP2[nMoveIdx];		// AXIS 19
	if (nAxis == AX_BTM_FOCUS_Z)		dMovePos = pMoveData->dBTMFocusZ[nMoveIdx];			// AXIS 20
	if (nAxis == AX_BTM_LIGHT_Z)		dMovePos = pMoveData->dBTMLightZ[nMoveIdx];			// AXIS 21
	if (nAxis == AX_TOP1_VISION_Z)		dMovePos = pMoveData->dTOPVisionZ1[nMoveIdx];		// AXIS 22
	if (nAxis == AX_TOP2_VISION_Z)		dMovePos = pMoveData->dTOPVisionZ2[nMoveIdx];		// AXIS 23
	if (nAxis == AX_INSPECT_STAGE_X1)	dMovePos = pMoveData->dVisionStageX1[nMoveIdx];		// AXIS 24
	if (nAxis == AX_INSPECT_STAGE_X2)	dMovePos = pMoveData->dVisionStageX2[nMoveIdx];		// AXIS 25
	if (nAxis == AX_INSPECT_STAGE_X3)	dMovePos = pMoveData->dVisionStageX3[nMoveIdx];		// AXIS 26
	if (nAxis == AX_INSPECT_STAGE_X4)	dMovePos = pMoveData->dVisionStageX4[nMoveIdx];		// AXIS 27
	if (nAxis == AX_UNLOAD_PICKER_X1)	dMovePos = pMoveData->dUnloadPickerX1[nMoveIdx];	// AXIS 28
	if (nAxis == AX_UNLOAD_PICKER_Y1)	dMovePos = pMoveData->dUnloadPickerY1[nMoveIdx];	// AXIS 29
	if (nAxis == AX_UNLOAD_PICKER_Z1)	dMovePos = pMoveData->dUnloadPickerZ1[nMoveIdx];	// AXIS 30
	if (nAxis == AX_UNLOAD_PICKER_P1)	dMovePos = pMoveData->dUnloadPickerP1[nMoveIdx];	// AXIS 31
	if (nAxis == AX_UNLOAD_PICKER_X2)	dMovePos = pMoveData->dUnloadPickerX2[nMoveIdx];	// AXIS 32
	if (nAxis == AX_UNLOAD_PICKER_Y2)	dMovePos = pMoveData->dUnloadPickerY2[nMoveIdx];	// AXIS 33
	if (nAxis == AX_UNLOAD_PICKER_Z2)	dMovePos = pMoveData->dUnloadPickerZ2[nMoveIdx];	// AXIS 34
	if (nAxis == AX_UNLOAD_PICKER_P2)	dMovePos = pMoveData->dUnloadPickerP2[nMoveIdx];	// AXIS 35
	if (nAxis == AX_GOOD_STAGE_Y1)		dMovePos = pMoveData->dGoodStageY1[nMoveIdx];		// AXIS 36
	if (nAxis == AX_GOOD_STAGE_Y2)		dMovePos = pMoveData->dGoodStageY2[nMoveIdx];		// AXIS 37
	if (nAxis == AX_NG_STAGE_Y1)		dMovePos = pMoveData->dNGStageY1[nMoveIdx];			// AXIS 38
	if (nAxis == AX_NG_STAGE_Y2)		dMovePos = pMoveData->dNGStageY2[nMoveIdx];			// AXIS 39
	if (nAxis == AX_SHIP_ALIGN_X)		dMovePos = pMoveData->dShiipAling[nMoveIdx];		// AXIS 40

	if (dMovePos == -999) return;
	if (nMoveIdx > 0 && (nAxis == AX_TRANSFER_Z2)) {
...
BOOL CCommon::Check_MoveData(int nAxis, int nMoveIdx, CString sData)
{
	...
	if (nAxis == AX_TRANSFER_X2)		dCheckPos = pMoveInput->dTransferX2[nMoveIdx];		// AXIS 08
	if (nAxis == AX_TRANSFER_Z2)		dCheckPos = pMoveInput->dTransferZ2[nMoveIdx];		// AXIS 09
	if (nAxis == AX_LOAD_STAGE_Y1)		dCheckPos = pMoveInput->dLoadStageY1[nMoveIdx];		// AXIS 10
	if (nAxis == AX_LOAD_STAGE_Y2)		dCheckPos = pMoveInput->dLoadStageY2[nMoveIdx];		// AXIS 11
	if (nAxis == AX_LOAD_PICKER_X1)		dCheckPos = pMoveInput->dLoadPickerX1[nMoveIdx];	// AXIS 12
	if (nAxis == AX_LOAD_PICKER_Y1)		dCheckPos = pMoveInput->dLoadPickerY1[nMoveIdx];	// AXIS 13
	if (nAxis == AX_LOAD_PICKER_Z1)		dCheckPos = pMoveInput->dLoadPickerZ1[nMoveIdx];	// AXIS 14
	if (nAxis == AX_LOAD_PICKER_P1)		dCheckPos = pMoveInput->dLoadPickerP1[nMoveIdx];	// AXIS 15
	if (nAxis == AX_LOAD_PICKER_X2)		dCheckPos = pMoveInput->dLoadPickerX2[nMoveIdx];	// AXIS 16
	if (nAxis == AX_LOAD_PICKER_Y2)		dCheckPos = pMoveInput->dLoadPickerY2[nMoveIdx];	// AXIS 17
	if (nAxis == AX_LOAD_PICKER_Z2)		dCheckPos = pMoveInput->dLoadPickerZ2[nMoveIdx];	// AXIS 18
	if (nAxis == AX_LOAD_PICKER_P2)		dCheckPos = pMoveInput->dLoadPickerP2[nMoveIdx];	// AXIS 19
	if (nAxis == AX_BTM_FOCUS_Z)		dCheckPos = pMoveInput->dBTMFocusZ[nMoveIdx];		// AXIS 20
	if (nAxis == AX_BTM_LIGHT_Z)		dCheckPos = pMoveInput->dBTMLightZ[nMoveIdx];		// AXIS 21
	if (nAxis == AX_TOP1_VISION_Z)		dCheckPos = pMoveInput->dTOPVisionZ1[nMoveIdx];		// AXIS 22
	if (nAxis == AX_TOP2_VISION_Z)		dCheckPos = pMoveInput->dTOPVisionZ2[nMoveIdx];		// AXIS 23
	if (nAxis == AX_INSPECT_STAGE_X1)	dCheckPos = pMoveInput->dVisionStageX1[nMoveIdx];	// AXIS 24
	if (nAxis == AX_INSPECT_STAGE_X2)	dCheckPos = pMoveInput->dVisionStageX2[nMoveIdx];	// AXIS 25
	if (nAxis == AX_INSPECT_STAGE_X3)	dCheckPos = pMoveInput->dVisionStageX3[nMoveIdx];	// AXIS 26
	if (nAxis == AX_INSPECT_STAGE_X4)	dCheckPos = pMoveInput->dVisionStageX4[nMoveIdx];	// AXIS 27
	if (nAxis == AX_UNLOAD_PICKER_X1)	dCheckPos = pMoveInput->dUnloadPickerX1[nMoveIdx];	// AXIS 28
	if (nAxis == AX_UNLOAD_PICKER_Y1)	dCheckPos = pMoveInput->dUnloadPickerY1[nMoveIdx];	// AXIS 29
	if (nAxis == AX_UNLOAD_PICKER_Z1)	dCheckPos = pMoveInput->dUnloadPickerZ1[nMoveIdx];	// AXIS 30
	if (nAxis == AX_UNLOAD_PICKER_P1)	dCheckPos = pMoveInput->dUnloadPickerP1[nMoveIdx];	// AXIS 31
	if (nAxis == AX_UNLOAD_PICKER_X2)	dCheckPos = pMoveInput->dUnloadPickerX2[nMoveIdx];	// AXIS 32
	if (nAxis == AX_UNLOAD_PICKER_Y2)	dCheckPos = pMoveInput->dUnloadPickerY2[nMoveIdx];	// AXIS 33
	if (nAxis == AX_UNLOAD_PICKER_Z2)	dCheckPos = pMoveInput->dUnloadPickerZ2[nMoveIdx];	// AXIS 34
	if (nAxis == AX_UNLOAD_PICKER_P2)	dCheckPos = pMoveInput->dUnloadPickerP2[nMoveIdx];	// AXIS 35
	if (nAxis == AX_GOOD_STAGE_Y1)		dCheckPos = pMoveInput->dGoodStageY1[nMoveIdx];		// AXIS 36
	if (nAxis == AX_GOOD_STAGE_Y2)		dCheckPos = pMoveInput->dGoodStageY2[nMoveIdx];		// AXIS 37
	if (nAxis == AX_NG_STAGE_Y1)		dCheckPos = pMoveInput->dNGStageY1[nMoveIdx];		// AXIS 38
	if (nAxis == AX_NG_STAGE_Y2)		dCheckPos = pMoveInput->dNGStageY2[nMoveIdx];		// AXIS 39
	if (nAxis == AX_SHIP_ALIGN_X)		dCheckPos = pMoveInput->dShiipAling[nMoveIdx];		// AXIS 40
...
void CCommon::Save_Motion(int nAxis, int nMoveIdx, double dTraget)
{
	...
	case  8: strAxis = "Transfer2,X"; break;
	case  9: strAxis = "Transfer2,Z"; break;
	case 10: strAxis = "LoadStage1,Y"; break;
	case 11: strAxis = "LoadStage2,Y"; break;
	case 12: strAxis = "LoadPicker1,X"; break;
	case 13: strAxis = "LoadPicker1,Y"; break;
	case 14: strAxis = "LoadPicker1,Z"; break;
	case 15: strAxis = "LoadPicker1,P"; break;
	case 16: strAxis = "LoadPicker2,X"; break;
	case 17: strAxis = "LoadPicker2,Y"; break;
	case 18: strAxis = "LoadPicker2,Z"; break;
	case 19: strAxis = "LoadPicker2,P"; break;
	case 20: strAxis = "BottomCamera,Z"; break;
	case 21: strAxis = "BottomLight,Z"; break;
	case 22: strAxis = "Top1,Z"; break;
	case 23: strAxis = "Top2,Z"; break;
	case 24: strAxis = "InspecStage1,X"; break;
	case 25: strAxis = "InspecStage2,X"; break;
	case 26: strAxis = "InspecStage3,X"; break;
	case 27: strAxis = "InspecStage4,X"; break;
	case 28: strAxis = "UnloadPicker1,X"; break;
	case 29: strAxis = "UnloadPicker1,Y"; break;
	case 30: strAxis = "UnloadPicker1,Z"; break;
	case 31: strAxis = "UnloadPicker1,P"; break;
	case 32: strAxis = "UnloadPicker2,X"; break;
	case 33: strAxis = "UnloadPicker2,Y"; break;
	case 34: strAxis = "UnloadPicker2,Z"; break;
	case 35: strAxis = "UnloadPicker2,P"; break;
	case 36: strAxis = "GoodStage1,Y"; break;
	case 37: strAxis = "GoodStage2,Y"; break;
	case 38: strAxis = "NgStage1,Y"; break;
	case 39: strAxis = "NgStage2,Y"; break;
	case 40: strAxis = "ShipAlign,X"; break;
	}
	...
	if (nMoveIdx != -1) {	// Check_Position 
		...
		case  8: dCheckPos = pMoveData->dTransferX2[nMoveIdx];		break;
		case  9: dCheckPos = pMoveData->dTransferZ2[nMoveIdx];		break;
		case 10: dCheckPos = pMoveData->dLoadStageY1[nMoveIdx];		break;
		case 11: dCheckPos = pMoveData->dLoadStageY2[nMoveIdx];		break;
		case 12: dCheckPos = pMoveData->dLoadPickerX1[nMoveIdx];	break;
		case 13: dCheckPos = pMoveData->dLoadPickerY1[nMoveIdx];	break;
		case 14: dCheckPos = pMoveData->dLoadPickerZ1[nMoveIdx];	break;
		case 15: dCheckPos = pMoveData->dLoadPickerP1[nMoveIdx];	break;
		case 16: dCheckPos = pMoveData->dLoadPickerX2[nMoveIdx];	break;
		case 17: dCheckPos = pMoveData->dLoadPickerY2[nMoveIdx];	break;
		case 18: dCheckPos = pMoveData->dLoadPickerZ2[nMoveIdx];	break;
		case 19: dCheckPos = pMoveData->dLoadPickerP2[nMoveIdx];	break;
		case 20: dCheckPos = pMoveData->dBTMFocusZ[nMoveIdx];		break;
		case 21: dCheckPos = pMoveData->dBTMLightZ[nMoveIdx];		break;
		case 22: dCheckPos = pMoveData->dTOPVisionZ1[nMoveIdx];		break;
		case 23: dCheckPos = pMoveData->dTOPVisionZ2[nMoveIdx];		break;
		case 24: dCheckPos = pMoveData->dVisionStageX1[nMoveIdx];	break;
		case 25: dCheckPos = pMoveData->dVisionStageX2[nMoveIdx];	break;
		case 26: dCheckPos = pMoveData->dVisionStageX3[nMoveIdx];	break;
		case 27: dCheckPos = pMoveData->dVisionStageX4[nMoveIdx];	break;
		case 28: dCheckPos = pMoveData->dUnloadPickerX1[nMoveIdx];	break;
		case 29: dCheckPos = pMoveData->dUnloadPickerY1[nMoveIdx];	break;
		case 30: dCheckPos = pMoveData->dUnloadPickerZ1[nMoveIdx];	break;
		case 31: dCheckPos = pMoveData->dUnloadPickerP1[nMoveIdx];	break;
		case 32: dCheckPos = pMoveData->dUnloadPickerX2[nMoveIdx];	break;
		case 33: dCheckPos = pMoveData->dUnloadPickerY2[nMoveIdx];	break;
		case 34: dCheckPos = pMoveData->dUnloadPickerZ2[nMoveIdx];	break;
		case 35: dCheckPos = pMoveData->dUnloadPickerP2[nMoveIdx];	break;
		case 36: dCheckPos = pMoveData->dGoodStageY1[nMoveIdx];		break;
		case 37: dCheckPos = pMoveData->dGoodStageY2[nMoveIdx];		break;
		case 38: dCheckPos = pMoveData->dNGStageY1[nMoveIdx];		break;
		case 39: dCheckPos = pMoveData->dNGStageY2[nMoveIdx];		break;
		case 40: dCheckPos = pMoveData->dShiipAling[nMoveIdx];		break;
		}
	} else {
		dCheckPos = dTraget;
	}
	...
	if (nAxis >= 12 && nAxis <= 14) {	// Load Picker1 X,Y,Z
	...
	if (nAxis >= 16 && nAxis <= 18) {	// Load Picker2 X,Y,Z
	...
	if (nAxis >= 24 && nAxis <= 27) {	// Inspection Stage X
		int nX = nAxis - 26;
	...
	if (nAxis >= 28 && nAxis <= 30) {	// Unload Picker1 X,Y,Z
	...
	if (nAxis >= 32 && nAxis <= 34) {	// Unload Picker2 X,Y,Z
	...
	strLotId = "";
	if (nAxis >=  7)				{ strLotId = gData.sLotID_Tansfer[0];		if (strLotId == "") strLotId = gData.sLotID_Tansfer[2]; }
	if (nAxis >=  8 && nAxis <= 9)	{ strLotId = gData.sLotID_Tansfer[1];		if (strLotId == "") strLotId = gData.sLotID_Tansfer[2]; }
	if (nAxis == 10)				{ strLotId = gData.sLotID_LoadStage[0];		if (strLotId == "") strLotId = gData.sLotID_LoadStage[2]; }
	if (nAxis == 11)				{ strLotId = gData.sLotID_LoadStage[1];		if (strLotId == "") strLotId = gData.sLotID_LoadStage[2]; }
	if (nAxis >= 12 && nAxis <= 15) { strLotId = gData.sLotID_LoadPicker[0];	if (strLotId == "") strLotId = gData.sLotID_LoadPicker[2]; }
	if (nAxis >= 16 && nAxis <= 19) { strLotId = gData.sLotID_LoadPicker[1];	if (strLotId == "") strLotId = gData.sLotID_LoadPicker[2]; }
	if (nAxis == 24)				{ strLotId = gData.sLotID_VisionStage[0];	if (strLotId == "") strLotId = gData.sLotID_VisionStage[4]; }
	if (nAxis == 25)				{ strLotId = gData.sLotID_VisionStage[1];	if (strLotId == "") strLotId = gData.sLotID_VisionStage[4]; }
	if (nAxis == 26)				{ strLotId = gData.sLotID_VisionStage[2];	if (strLotId == "") strLotId = gData.sLotID_VisionStage[4]; }
	if (nAxis == 27)				{ strLotId = gData.sLotID_VisionStage[3];	if (strLotId == "") strLotId = gData.sLotID_VisionStage[4]; }
	if (nAxis >= 28 && nAxis <= 31) { strLotId = gData.sLotID_UnloadPicker[0];	if (strLotId == "") strLotId = gData.sLotID_UnloadPicker[2]; }
	if (nAxis >= 32 && nAxis <= 35) { strLotId = gData.sLotID_UnloadPicker[1];	if (strLotId == "") strLotId = gData.sLotID_UnloadPicker[2]; }
	if (nAxis == 36)				{ strLotId = gData.sLotID_GoodTray[0];		if (strLotId == "") strLotId = gData.sLotID_GoodTray[2]; }
	if (nAxis == 37)				{ strLotId = gData.sLotID_GoodTray[1];		if (strLotId == "") strLotId = gData.sLotID_GoodTray[2]; }
	if (nAxis == 38)				{ strLotId = gData.sLotID_NGTray[0];		if (strLotId == "") strLotId = gData.sLotID_NGTray[2]; }
	if (nAxis == 39)				{ strLotId = gData.sLotID_NGTray[1];		if (strLotId == "") strLotId = gData.sLotID_NGTray[2]; }
